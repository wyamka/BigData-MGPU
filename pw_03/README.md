...
- Spark MLlib 
- Scikit-learn (sklearn): Оценка качества моделей (Confusion Matrix, ROC-кривая, AUC).
- UDF (PySpark): Пользовательские функции для создания признаков (hr_trend, hr_rolling_std).
- StandardScaler: Нормализация данных ($\mu=0, \sigma=1$) средствами MLlib.
...
Matplotlib: Основной инструмент для построения графиков и визуализации результатов.
Seaborn: Создание тепловых карт (Heatmaps) для анализа результатов Grid Search.
